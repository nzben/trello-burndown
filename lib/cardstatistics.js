			...
			if (card.actions) {
				for (var idxActions = 0; idxActions < card.actions.length; idxActions++) {
					if (card.actions[idxActions]) {
						// if we have any action, and this action is a move to a relevant list, log it
						if(card.actions[idxActions].data.listAfter && lists.indexOf(card.actions[idxActions].data.listAfter.name) > -1 )
						{
							var movedTo = card.actions[idxActions].data.listAfter;
							...
							if(!data.cumulativeFlow.length)
							{
								...
								data.cumulativeFlow[0] = { date: cleanDate, list: movedTo.name, estimate: estimate, effort: effort };
							} else {
								...
								for (var idxListEffort = data.cumulativeFlow.length - 1; idxListEffort >= 0; idxListEffort--) {
									if(Date.parse(data.cumulativeFlow[idxListEffort].date) === Date.parse(cleanDate)
										&& data.cumulativeFlow[idxListEffort].list == movedTo.name) {
								...
								if(!found) {
									...
									data.cumulativeFlow.push({ date: cleanDate, list: movedTo.name, estimate: estimate, effort: effort });
								}
							}
						}
