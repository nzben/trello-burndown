...
main.use('/api/sprint', function(req, res, next) {
	if (req.method === "GET" && req.query.sprint) {
      ...
      var boardsSplitted = data.boards.split(',');
      for (var i = 0; i < boardsSplitted.length; i++) {
        result.boards.push({ name: boardsSplitted[i] });
      }
...
main.use('/api/boards', function(req, res, next) {
	if (req.method === "GET") {
		var helper = new Helper();
		helper.getBoardList(function(err, data) {
			if (err) {
				res.writeHead(500);
				return res.end();
			}

			console.log('got board data: ' + data);
			res.writeHead(200);
...
function getConfigFromJson(POST) {
    ...
    for (var i = 0; i < POST.boards.length; i++) {
      config.boards += POST.boards[i].name;
      if (i < (POST.boards.length - 1))
        config.boards += ',';
    }
...
main.use('/', function(req, res, next) {
    ...
    var sprintDataCumulativePath = path.join(settings.exportPath, sprint + 'Cumulative.json');
    ...
    if(fs.existsSync(sprintDataCumulativePath)) {
      sprintData.cumulative = JSON.parse(fs.readFileSync(sprintDataCumulativePath, 'utf-8'));
    } else {
      sprintData.cumulative = {};
    }
...
function generateOutput(sprint, sprintData, sprintDataPath, callback) {
    ...
    cumulativeFlow: generateCumulativeFlow(sprintData.cumulative)
	};
...
function generateBurndown(sprintData) {
    ...
    {	
    	...
    	var newData1 = {
    		x: new Date(Date.parse(lineData.date)).getDate(),
        //parseInt(lineData.date.getUTCDate()),
    		y: parseFloat(lineData.idealEstimate)
    	};
    	var newData2 = {
        x: new Date(Date.parse(lineData.date)).getDate(),
    		//x: parseInt(lineData.day.getUTCDate()),
...
function generateCumulativeFlow(cumulativeFlowData) {
  var data = { labels: [], lines: [] };

  for (var i = 0; i < cumulativeFlowData[0].listData.length; i++) {
    data.labels.push(cumulativeFlowData[0].listData[i].list);
  };


  for (var line = 0; line < cumulativeFlowData.length; line++)
  {
    var lineData = cumulativeFlowData[line];
    data.lines[line] = {};
    data.lines[line].x = new Date(Date.parse(lineData.date)).getDate();

    for (var listLineIDX = 0; listLineIDX < lineData.listData.length; listLineIDX++) {
      data.lines[line]['data'+listLineIDX] = parseFloat(lineData.listData[listLineIDX].estimate)
    };
  }
  console.log(data);
  return data;
