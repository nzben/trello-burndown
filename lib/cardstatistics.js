...
CardStatistics.prototype.generate = function(cards, lists, finishLists, standuptime, callback) {
	var data = {
		"estimate": 0,
		"estimatedone": 0,
		"efforttotal": 0,
			...
			if (card.actions) {
				for (var idxActions = 0; idxActions < card.actions.length; idxActions++) {
					if (card.actions[idxActions]) {
						// if we have any action, and this action is a move FROM a relevant list, log it
						if(card.actions[idxActions].data.listBefore && lists.indexOf(card.actions[idxActions].data.listBefore.name) > -1 )
						{
							var movedFrom = card.actions[idxActions].data.listBefore;
							var date = new Date(Date.parse(card.actions[idxActions].date));
							var cleanDate = getRelatingDay(date, standup);

							if(!data.cumulativeFlow.length)
							{
								// first entry, just add it
								data.cumulativeFlow[0] = { date: cleanDate, list: movedFrom.name, estimate: estimate, effort: effort };
							} else {
								// see if we have an entry for this date and list
								var found = false;

								for (var idxListEffort = data.cumulativeFlow.length - 1; idxListEffort >= 0; idxListEffort--) {
									if(Date.parse(data.cumulativeFlow[idxListEffort].date) === Date.parse(cleanDate)
										&& data.cumulativeFlow[idxListEffort].list == movedFrom.name) {
										//we already have an entry for this list on this date, so increment estimate
										data.cumulativeFlow[idxListEffort].estimate += estimate;
										data.cumulativeFlow[idxListEffort].effort += effort;
										found = true;
									}
								};
								if(!found) {
									// no entry for this date & list so add a new entry
									data.cumulativeFlow.push({ date: cleanDate, list: movedFrom.name, estimate: estimate, effort: effort });
								}
							}
						}
	...
	console.log("BUILT "+data.cumulativeFlow.length+" cumulativeFlow records");
	callback(null, data);
}
...
CardStatistics.prototype.export = function(data, resources, days, name, lists, callback) {
	var statsData = [];
	...
	for (var date = 0; date < days.length; date++) {
		...
		var cumeContent = getDateData(dateToReceive, data.cumulativeFlow);
		...
		if(!cumulativeFlowData[date])
		{
			cumulativeFlowData[date] = { day: date, date: dateToReceive, listData: [] };
		}

		// iterate all lists, and look for any data on that list
		for (var listIDX = 0; listIDX < splittedLists.length; listIDX++) {
			cumulativeFlowData[date].listData.push( { list: splittedLists[listIDX], estimate: 0, effort: 0});
			for (var cumeIdx = 0; cumeIdx < cumeContent.length; cumeIdx++) {
				if(cumeContent[cumeIdx].list == splittedLists[listIDX])
				{
					cumulativeFlowData[date].listData[cumulativeFlowData[date].listData.length-1].estimate = cumeContent[cumeIdx].estimate;
					cumulativeFlowData[date].listData[cumulativeFlowData[date].listData.length-1].effort = cumeContent[cumeIdx].effort;
				}
			};
		};
	...
	var statsCumeExportResults = saveJSON(settings.exportPath, cumulativeFlowData, name + "Cumulative");

	if (statsExportResult)  {
		callback(statsExportResult);
	} else if (statsExExportResult) {
		callback(statsExExportResult);
	} else if (statsCumeExportResults){
		callback(statsCumeExportResults);
	} else {
		callback();
	}
}
