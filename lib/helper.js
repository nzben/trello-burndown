...
Helper.prototype.saveConfiguration = function(boards, lists, days, resources, finishedList, standupTime, name) {
	var configuration = {
		boards: boards,
		lists: lists,
		days: days,
...
Helper.prototype.generateAndExportFromConfig = function(name, generateAndExportFromConfig_callback) {
	...
	if (fs.existsSync(configPath)) {
		...
		if (!this.isSprintActive(name)) {
			...
			this.generateAndExport(configuration.boards, configuration.lists, configuration.days, configuration.resources, configuration.finishedList, configuration.standupTime, configuration.name, generateAndExportFromConfig_callback);
		}
	} else {
...
Helper.prototype.generateAndExport = function(boards, lists, days, resources, finishedLists, standuptime, name, callback) {
	var receiver = new CardReceiver(settings.applicationKey, settings.userToken, settings.boardId);

	var splittedLists = lists.split(',');
	var splittedBoards = boards.split(',');

	receiver.receive(splittedBoards, splittedLists, function(err, cards) {
		if (err) {
			...
			statistics.generate(cards, lists, finishedLists, standuptime, function(err, data) {
				if (err) {
					console.log(err);
				} else {
					printStatistics(data);

					statistics.export(data, resources, days, name, lists, function(error) {
						if (error) {
							if (callback) {
								callback(error);
							} else {
